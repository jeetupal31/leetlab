...
const CreateProblemForm = () => {
  const { createProblem, isProblemLoading: isLoading } = useProblemStore();
  ...
  const loadSampleData = () => {
    const sample = {
      ...
      hints: "Use backtracking to try different numbers in empty cells.",
      editorial: "The problem can be solved using a standard backtracking algorithm.",
      codeSnippets: {
      ...
      referenceSolutions: {
        JAVASCRIPT: "var solveSudoku = function(board) { /* JS Solution */ };",
        PYTHON: "class Solution: def solveSudoku(self, board): pass",
        JAVA: "class Solution { public void solveSudoku(char[][] board) {} }",
      },
      examples: {
        JAVASCRIPT: { input: "Sudoku Board", output: "Solved Sudoku Board", explanation: "Backtracking fills numbers." },
        PYTHON: { input: "Sudoku Board", output: "Solved Sudoku Board", explanation: "Backtracking fills numbers." },
        JAVA: { input: "Sudoku Board", output: "Solved Sudoku Board", explanation: "Backtracking fills numbers." }
      }
    };
    Object.entries(sample).forEach(([key, val]) => setValue(key, val));
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
          <div>
            <h1 className="text-3xl font-black text-gray-900 flex items-center gap-3">
              ...
              Create <span className="text-blue-600">New Problem</span>
            </h1>
            <p className="mt-2 text-gray-500 font-medium">Design a high-quality challenge for the community.</p>
          </div>
          <button
            type="button"
            onClick={loadSampleData}
            className="group flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-700 px-6 py-4 rounded-2xl font-bold border-2 border-dashed border-gray-200 transition-all hover:border-blue-400"
          >
            <Sparkles className="w-5 h-5 text-amber-500 group-hover:scale-110" />
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-3">
                    ...
                    />
                    {errors.title && <p className="text-red-500 text-xs font-bold uppercase track-wide ml-2">{errors.title.message}</p>}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <div className="flex flex-wrap gap-2 p-6 bg-gray-50 rounded-3xl border border-gray-100">
                        {["Array", "String", "Hash Table", "Dynamic Programming", "Math", "Sorting", "Greedy", "DFS", "Binary Search", "Two Pointers", "BFS", "Tree", "Matrix", "Backtracking", "Stack", "Design", "Graph", "Heap"].map((tag) => (
                          <button
                            key={tag}
                            type="button"
                            onClick={() => {
                  ...
                  />
                  {errors.tags && <p className="text-red-500 text-xs font-bold uppercase track-wide ml-2">{errors.tags.message}</p>}
                </div>
              </motion.div>
            )}
                ...
                <div className="space-y-6">
                  ...
                  {fields.length === 0 && (
                    <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-[2rem]">
                      ...
                      <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">No test cases added yet</p>
                    </div>
                  )}
                </div>
              </motion.div>
